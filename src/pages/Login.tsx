...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      let isComplete = hasCompletedOnboarding(result.user.uid);
      
      try {
        const syncResponse = await syncUser();
        // Fallback to local check if backend is somehow down. If backend responds, sync local with it.
        if (syncResponse?.success && syncResponse.data) {
          isComplete = syncResponse.data.onboardingCompleted;
        }
      } catch (e) {
