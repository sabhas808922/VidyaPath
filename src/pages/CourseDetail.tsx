...
const lessonIcons: Record<string, React.ElementType> = { video: Video, reading: FileText, quiz: HelpCircle, pdf: FileText };
            ...
            <AnimatePresence mode="wait">
              {!activeL ? (
                ...
                >
                  ...
                  {activeL.type === "video" && (
                    <div className="mb-6 flex w-full aspect-video items-center justify-center overflow-hidden rounded-lg bg-muted">
                      {activeL.content.startsWith("http") ? (
                        <iframe src={activeL.content} className="h-full w-full border-0" allowFullScreen></iframe>
                      ) : (
                  ...
                  {activeL.type === "pdf" && (
                    <div className="mb-6 flex w-full aspect-[1/1.2] items-center justify-center overflow-hidden rounded-lg bg-muted">
                        <iframe src={activeL.content} className="h-full w-full border-0" title="PDF Document"></iframe>
                    </div>
                  )}

                  <div className="prose prose-sm max-w-none">
                    {activeL.type !== "pdf" && !activeL.content.startsWith("http") && activeL.content.split("\n").map((line, i) => (
