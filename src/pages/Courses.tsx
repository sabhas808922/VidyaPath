...
import { classGroups, getOnboardingProfile } from "@/data/onboarding";
...
const Courses = () => {
  const { user } = useAuth();
  const onboardedClass = user ? getOnboardingProfile(user.uid)?.classGroup : null;
  
  const [search, setSearch] = useState("");
  const [selectedClass, setSelectedClass] = useState<string>(onboardedClass || classGroups[0].value);
  const [selectedSubject, setSelectedSubject] = useState<string>("All");

  const classCourses = courses.filter((c) => c.targetClassGroup === selectedClass);
  const subjects = ["All", ...new Set(classCourses.map((c) => c.subject))];
